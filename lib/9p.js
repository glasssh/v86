    ...
    switch(id)
    {
        ...
        case 116: // read
            ...
            if (this.fids[fid].type == FID_XATTR) {
                ...
                if(data) {
                    // if data doesn't fit into reply buffer, expand the reply buffer to fit
                    if(data.length > this.replybuffer.length - (7 + 4)){
                        this.replybuffer = new Uint8Array(data.length + 7 + 4);
                        this.replybuffersize = 0;
                    }
